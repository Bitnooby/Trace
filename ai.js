module.exports = function ai({ redisOn, redisCmd } = {}) {
  ...
  const prompt = (caption, evidence) =>
    ...
    'Critical: you are usually seeing a SINGLE STILL FRAME pulled from a short social video with no provenance. Modern AI video generators (Sora, Veo, Kling, Runway) make frames that look fully photorealistic in a still even when the entire clip is synthetic, so the ABSENCE of visible tells does NOT mean the video is real. Stay calibrated and skeptical: reserve high "Likely authentic" confidence for frames showing positive signs of real capture (sensor noise or grain, natural depth-of-field, motion blur, mundane incidental detail). For novel, futuristic, or implausible viral "feats" (a robot doing a human task, an impossible stunt, an "oddly satisfying" too-perfect scene), lean to "Inconclusive" or "Likely AI-generated" with modest confidence, and state plainly that a single clean frame cannot rule out an AI-generated video. ' +
