...
app.get('/sitemap.xml', async (req, res) => {
  const base = `${req.protocol}://${req.get('host')}`;
  const pages = ['/', '/feed', '/radar', '/onrecord', '/trending', '/about', '/faq', '/terms', '/why-ai-video-detectors-fail', '/developers', '/privacy'];
  let reports = [];
...
app.get('/radar', async (req, res) => {
  ...
  if (view === 'cleared') {
    ...
    const cbody = `<div class="rad"><div class="rad-head"><div class="rad-eyebrow">Relity Radar</div><h1 class="rad-h1">What’s cleared right now</h1><p class="rad-sub">Media that cleared Relity’s checks — authentic-capture signals (on news outlets or carrying camera/provenance data), with no debunk on record ${span}. Still <b>evidence, not a verdict</b>: this reads the media, not the truth of any caption.</p>${toggle}</div><div class="rad-feed">${feed}</div><div class="rad-foot">Updates as new posts clear. <a href="${base}/trending">See the full feed →</a> · <a href="https://x.com/intent/tweet?text=${encodeURIComponent('🔎 Relity Radar — what is circulating right now: confirmed signals vs unverified noise. Evidence, not a verdict:')}&url=${encodeURIComponent(base+'/radar')}" target="_blank" rel="noopener noreferrer">Share on X →</a></div><a class="cta" href="${base}/" style="max-width:320px;margin:8px auto 0">Check something →</a></div>`;
    return res.send(page('Relity Radar — what’s cleared', cbody, base, og, true));
  }
  const body=total?`<div class="rad"><div class="rad-head"><div class="rad-eyebrow">Relity Radar</div><h1 class="rad-h1">What’s circulating right now</h1><p class="rad-sub">The <b>“is it real?”</b> lens at scale — a live, aggregate read on the viral images and clips people have run through Relity ${span}. <b>Evidence, not a verdict</b> — each item is one automated read; open any to judge for yourself.</p>${toggle}</div><div class="rad-pulse"><div class="rad-pulse-top"><span class="rad-total">${total}</span> check${total===1?'':'s'} analyzed ${span}</div><div class="rad-bar">${bar}</div><div class="rad-legend">${legend}</div></div>${srcLine}<div class="rad-grid">${sections}</div><div class="rad-foot">Updates as new posts are checked. <a href="${base}/trending">See the full feed →</a> · <a href="https://x.com/intent/tweet?text=${encodeURIComponent('🔎 Relity Radar — what is circulating right now: confirmed signals vs unverified noise. Evidence, not a verdict:')}&url=${encodeURIComponent(base+'/radar')}" target="_blank" rel="noopener noreferrer">Share on X →</a></div><a class="cta" href="${base}/" style="max-width:320px;margin:8px auto 0">Check something →</a></div>`:`<div class="rad"><div class="rad-head"><div class="rad-eyebrow">Relity Radar</div><h1 class="rad-h1">What’s circulating right now</h1><p class="rad-sub">The radar fills as viral posts get checked — nothing has crossed the threshold yet. <a href="${base}/">Run a check</a> to start the board.</p></div></div>`;
...
const SITE_FOOTER = "<footer style=\"background:#0C1A1A;margin-top:72px\"><div style=\"max-width:960px;margin:0 auto;padding:46px 22px 26px\"><div style=\"display:flex;flex-wrap:wrap;gap:32px 40px;justify-content:space-between\"><div style=\"max-width:270px;min-width:200px\"><div style=\"display:flex;align-items:center;gap:9px;margin-bottom:12px\"><span style=\"width:30px;height:30px;border-radius:8px;background:linear-gradient(150deg,#0B6E6E,#13A8A8);display:inline-flex;align-items:center;justify-content:center\"><svg viewBox=\"0 0 100 100\" width=\"20\" height=\"20\" fill=\"none\" xmlns=\"http://www.w3.org/2000/svg\"><g stroke=\"#fff\" stroke-width=\"8\" stroke-linecap=\"round\"><line x1=\"32\" y1=\"34\" x2=\"50\" y2=\"52\"/><line x1=\"50\" y1=\"52\" x2=\"70\" y2=\"36\"/><line x1=\"50\" y1=\"52\" x2=\"52\" y2=\"78\"/></g><circle cx=\"32\" cy=\"34\" r=\"8\" fill=\"#fff\"/><circle cx=\"70\" cy=\"36\" r=\"8\" fill=\"#fff\"/><circle cx=\"52\" cy=\"78\" r=\"8\" fill=\"#fff\"/><circle cx=\"50\" cy=\"52\" r=\"9.5\" fill=\"#fff\"/></svg></span><span style=\"font-family:'Space Grotesk',system-ui,sans-serif;font-weight:700;letter-spacing:.16em;font-size:16px;color:#fff\">RELITY</span></div><p style=\"font-size:13px;line-height:1.6;color:#8FB0B0;margin:0 0 16px\">Evidence, not verdicts. Check anything \u2014 is it real? \u2014 and see what newsrooms actually confirm. You decide, not an algorithm.</p><div style=\"display:flex;gap:9px\"><a href=\"https://x.com/RelityAi\" target=\"_blank\" rel=\"noopener\" aria-label=\"X\" style=\"display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;background:#14302e;border:1px solid #1E3A3A\"><svg viewBox=\"0 0 24 24\" width=\"16\" height=\"16\" fill=\"#9FD8D8\"><path d=\"M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932 6.064-6.933ZM17.61 20.644h2.039L6.486 3.24H4.298L17.61 20.644Z\"/></svg></a><a href=\"https://www.instagram.com/relity.ai\" target=\"_blank\" rel=\"noopener\" aria-label=\"Instagram\" style=\"display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;background:#14302e;border:1px solid #1E3A3A\"><svg viewBox=\"0 0 24 24\" width=\"17\" height=\"17\" fill=\"none\" stroke=\"#9FD8D8\" stroke-width=\"2\"><rect x=\"2.5\" y=\"2.5\" width=\"19\" height=\"19\" rx=\"5.4\"/><circle cx=\"12\" cy=\"12\" r=\"4.3\"/><circle cx=\"17.6\" cy=\"6.4\" r=\"1.1\" fill=\"#9FD8D8\" stroke=\"none\"/></svg></a><a href=\"https://www.facebook.com/1219929064533002\" target=\"_blank\" rel=\"noopener\" aria-label=\"Facebook\" style=\"display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;background:#14302e;border:1px solid #1E3A3A\"><svg viewBox=\"0 0 24 24\" width=\"16\" height=\"16\" fill=\"#9FD8D8\"><path d=\"M24 12.07C24 5.4 18.63 0 12 0S0 5.4 0 12.07c0 6.02 4.39 11.01 10.13 11.93v-8.44H7.08v-3.49h3.05V9.41c0-3.02 1.79-4.69 4.53-4.69 1.31 0 2.68.24 2.68.24v2.97h-1.51c-1.49 0-1.96.93-1.96 1.89v2.25h3.33l-.53 3.49h-2.8v8.44C19.61 23.08 24 18.09 24 12.07Z\"/></svg></a><a href=\"https://t.me/RelityCheck_bot\" target=\"_blank\" rel=\"noopener\" aria-label=\"Telegram\" style=\"display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:9px;background:#14302e;border:1px solid #1E3A3A\"><svg viewBox=\"0 0 24 24\" width=\"17\" height=\"17\" fill=\"#9FD8D8\"><path d=\"M9.78 18.65l.28-4.23 7.68-6.92c.34-.31-.07-.46-.52-.19L7.74 13.3 3.64 12c-.88-.25-.89-.86.2-1.3l15.97-6.16c.73-.33 1.43.18 1.15 1.3l-2.72 12.81c-.19.91-.74 1.13-1.5.71L12.6 16.3l-1.99 1.93c-.23.23-.42.42-.83.42Z\"/></svg></a></div></div><div><h4 style=\"font-family:'Space Grotesk',system-ui,sans-serif;font-weight:600;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#5FB8B8;margin:0 0 8px\">Check</h4><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/\">Is it real?</a><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/feed\">News feed</a><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/radar\">Radar</a><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/onrecord\">On the Record</a><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/trending\">Trending checks</a><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/developers\">API &amp; developers</a></div><div><h4 style=\"font-family:'Space Grotesk',system-ui,sans-serif;font-weight:600;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#5FB8B8;margin:0 0 8px\">Learn</h4><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/about\">About</a><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/faq\">FAQ</a><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/why-ai-video-detectors-fail\">Why AI video fools detectors</a><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/history\">Your history</a></div><div><h4 style=\"font-family:'Space Grotesk',system-ui,sans-serif;font-weight:600;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#5FB8B8;margin:0 0 8px\">Legal</h4><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/privacy\">Privacy</a><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"/terms\">Terms of Use</a><a style=\"color:#9FD8D8;text-decoration:none;font-size:13.5px;line-height:2.05;display:block\" href=\"mailto:hello@relity.ai\">Contact</a></div></div><div style=\"border-top:1px solid #1E3A3A;margin-top:32px;padding-top:18px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px\"><span style=\"font-size:12.5px;color:#6FA0A0\">\u00a9 2026 Relity \u00b7 Evidence, not verdicts.</span><span style=\"font-size:12.5px;color:#6FA0A0\">Corroboration is breadth of reporting, not proof \u2014 you decide.</span></div></div></footer>";
function siteFooter(){ return SITE_FOOTER; }
app.get('/faq', (req,res)=>{
  const base=`${req.protocol}://${req.get('host')}`;
  const qa=(q,a)=>`<h2 style="font-family:'Space Grotesk',system-ui,sans-serif;font-weight:700;font-size:17px;color:var(--ink);margin:24px 0 6px">${q}</h2><p style="color:var(--g);font-size:14.5px;line-height:1.6;margin:0">${a}</p>`;
  const body=`<h1 style="font-family:'Space Grotesk',system-ui,sans-serif;font-weight:700;font-size:30px;color:var(--ink);margin:6px 0 6px">FAQ</h1><p style="color:var(--g);font-size:14.5px;margin:0 0 8px">What Relity does — and what it deliberately doesn't.</p>`
   + qa(`What is Relity?`, `Paste any image, video, link, or claim and Relity shows you the evidence behind it — where it came from, AI-generation signals, where it has appeared online, and whether the caption actually holds up. Then <b>you</b> decide. It also surfaces what multiple independent newsrooms are corroborating right now.`)
   + qa(`What does "evidence, not verdicts" mean?`, `We never stamp "FAKE" or "TRUE." We show the forensic signals and context, weighed and explained in plain language, and leave the judgment to you — the human.`)
   + qa(`Is it free?`, `Yes — free to use, no sign-up. Relity Pro adds unlimited web cross-checks and a dual AI read (Claude + Gemini).`)
   + qa(`Is my data private?`, `Image file checks run in your browser. We only send the single item you choose, when you choose it, to generate a report — no tracking, no browsing-history collection.`)
   + qa(`How accurate is it?`, `It surfaces <i>signals</i>, not certainties. Missing camera data does not prove a fake — screenshots and re-saves strip it. Present data can be forged. Weigh the signals; don't hand the judgment to a machine.`)
   + qa(`What is "On the Record"?`, `Verbatim posts from influential public figures, pulled from their verified accounts and linked to the original — no sides, a verified statement from across the spectrum. The primary source, before it becomes a copy of a copy.`)
   + qa(`What is the news feed?`, `Stories ranked by how many independent newsrooms carry each one. Corroboration is breadth of reporting, not proof of truth — it shows how widely a story is reported, then you read it yourself.`)
   + qa(`Where can I use Relity?`, `On the web at relity.ai, via the browser extension (Chrome and Firefox), and the Telegram bot @RelityCheck_bot.`);
  res.send(page('Relity — FAQ', body, base, null, false));
});
app.get('/terms', (req,res)=>{
  const base=`${req.protocol}://${req.get('host')}`;
  const sec=(h,t)=>`<h2 style="font-family:'Space Grotesk',system-ui,sans-serif;font-weight:700;font-size:17px;color:var(--ink);margin:24px 0 6px">${h}</h2><p style="color:var(--g);font-size:14.5px;line-height:1.6;margin:0">${t}</p>`;
  const body=`<h1 style="font-family:'Space Grotesk',system-ui,sans-serif;font-weight:700;font-size:30px;color:var(--ink);margin:6px 0 6px">Terms of Use</h1><p style="color:var(--g);font-size:13.5px;margin:0 0 8px">Last updated June 30, 2026</p>`
   + sec(`1. Acceptance`, `By accessing or using Relity (relity.ai, its extensions, bots, and API), you agree to these Terms. If you do not agree, please do not use the service.`)
   + sec(`2. What Relity is`, `Relity is an evidence and media-verification tool. It surfaces forensic signals, provenance, corroboration, and primary sources so you can reach your own conclusions. <b>Relity does not provide verdicts, and it is not legal, financial, journalistic, or professional advice.</b> You are responsible for how you interpret and use what it shows you.`)
   + sec(`3. No warranty`, `The service is provided "as is" and "as available," without warranties of any kind. Signals may be incomplete, delayed, or wrong; absence of a signal is not proof, and presence of one is not certainty. We do not guarantee accuracy, availability, or fitness for any particular purpose.`)
   + sec(`4. Acceptable use`, `Do not use Relity to harass, defame, deceive, or for any unlawful purpose, and do not abuse, overload, or attempt to disrupt the service or API. We may rate-limit or suspend access to protect the service.`)
   + sec(`5. Primary-source content`, `"On the Record" reproduces public figures' own public posts verbatim, with a link to the original, for transparency. Inclusion is not endorsement, and statements are the speakers' own. If an original is later deleted, the link may reflect that.`)
   + sec(`6. Third-party content & links`, `Reports may reference third-party sources, outlets, and platforms. That content belongs to its owners and is subject to their terms; Relity is not responsible for it.`)
   + sec(`7. Intellectual property`, `The Relity name, brand, and site are ours. You may view and share the reports Relity generates; you may not misrepresent them or strip their context.`)
   + sec(`8. Changes`, `We may update these Terms from time to time. Continued use after a change means you accept the updated Terms.`)
   + sec(`9. Contact`, `Reach us on X (<a href="https://x.com/RelityAi" target="_blank" rel="noopener" style="color:var(--signal);text-decoration:none">@RelityAi</a>) or the channels in our footer.`)
   + `<p style="color:#8A95A4;font-size:12.5px;line-height:1.6;margin:26px 0 0">This is a plain-language template provided for convenience and is not legal advice. For a binding agreement, have it reviewed by a qualified lawyer.</p>`;
  res.send(page('Relity — Terms of Use', body, base, null, false));
});
app.get('/about', (req,res)=>{
  const base=`${req.protocol}://${req.get('host')}`;
  const p=t=>`<p style="color:var(--g);font-size:15px;line-height:1.65;margin:0 0 14px">${t}</p>`;
  const h=t=>`<h2 style="font-family:'Space Grotesk',system-ui,sans-serif;font-weight:700;font-size:18px;color:var(--ink);margin:26px 0 8px">${t}</h2>`;
  const body=`<h1 style="font-family:'Space Grotesk',system-ui,sans-serif;font-weight:700;font-size:30px;color:var(--ink);margin:6px 0 12px">About Relity</h1>`
   + p(`The internet is full of convincing fakes — and real events buried under noise. Most tools "fix" this by handing you a verdict and asking you to trust it. <b>Relity does the opposite: it shows you the evidence, and lets you decide.</b>`)
   + h(`Two questions, one method`)
   + p(`<b>Is it real?</b> — paste any image, video, post, or claim, and Relity shows its provenance, AI-generation signals, where it has appeared online, and whether the caption actually holds up.`)
   + p(`<b>What's real?</b> — see the stories multiple independent newsrooms are corroborating right now, and read primary-source statements from public figures verbatim, linked to the original.`)
   + p(`Same rule both ways: the evidence, weighed — you make the call. We never stamp "FAKE" or "TRUE."`)
   + h(`Why "evidence, not verdicts"`)
   + p(`A black-box "this is fake" is just another thing to trust. Signals can be incomplete or forged; missing metadata doesn't prove a fake, and its presence doesn't prove authenticity. So we show the receipts and the reasoning, and leave the judgment where it belongs — with you.`)
   + p(`Relity is free, no sign-up — on the web, as a browser extension, and a Telegram bot. <a href="${base}/" style="color:var(--signal);font-weight:600;text-decoration:none">Check something →</a>`);
  res.send(page('Relity — About', body, base, null, false));
});
function page(title, body, base, og, wide) {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1"/><title>${title}</title><link rel="icon" href="/favicon.ico" sizes="any"/><link rel="icon" type="image/svg+xml" href="/favicon.svg"/><link rel="preconnect" href="https://fonts.googleapis.com"/><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin/><link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&family=Inter:wght@400;450;500;600&family=IBM+Plex+Mono:wght@400;500;600&display=swap" rel="stylesheet"/>${og||''}<script type="application/ld+json">{"@context":"https://schema.org","@graph":[{"@type":"Organization","name":"Relity","url":"https://relity.ai","logo":"https://relity.ai/icon-512.png","description":"Relity shows the evidence behind images, videos and claims so you can decide — provenance, where it appears, fact-checks and AI signals. Evidence, not a verdict."},{"@type":"WebSite","name":"Relity","url":"https://relity.ai"}]}</script>
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:Inter,system-ui,sans-serif;line-height:1.5}
    ...
    .fd-side-h{font-family:'Space Grotesk',system-ui,sans-serif;font-weight:700;font-size:12px;letter-spacing:.06em;text-transform:uppercase;color:var(--signal);margin:0 0 2px;padding-bottom:9px;border-bottom:2px solid var(--signal-wash,#C5E5E5)}
